...
# Data and Analysis
  ...
  The data utilized in this exploration are snowshoe hare trap data from the Bonanza Creek LTER taken from 1998 to 2012 (Kielland et al., 2017). Following are basic analyses of the juvinile snowshoe hare population. These analyses inculde explorations of population through time, weight comparisons by sex, and weight and hind foot comparisons. Methods utilized include `r weight_ttest$method`, `r weight_effect$method`, and basic linear regression modeling in addition to basic graphical visualization.
...
```
  ...
  From 1998 to 2012, an average of `r juvenile_summary[1:1]` individuals were trapped annually with a maximum of `r juvenile_summary[1:4]` individuals and a minimum of `r juvenile_summary[1:5]` individuals which indicates that the measure of central tendancy does not accurately represent the data. In part this is because over time the number of individuals trapped annually decreased (Figure 1). However, it is also important to note that these measures utilized absoulte counts, so further analyses should standardize the counts by dividing by the total days of trapping in each year to ensure that the effort signal does not introduce variance that does not exist. 
...
```
  ...
  Hares trapped at the general site (bonbs) were larger on average than at the other two sites; however, there were more individuals and a larger spread trapped at the riparian site (bonrip). Across all trap sites, juvenile male showshoe hare are larger on average than female juveniles. This difference is most defined in the mature forest trap site (bonmat) and least defined in the general trap site (bonbs)(Figure 2). 
